...
int kontrolaKljuca(int igra)
{
    ...
    if (found)
    {
        ...
        if ((key[i].key_type == SAT && key[i].time_spent < SAT) || (key[i].key_type == DAN && key[i].time_spent < DAN) || (key[i].key_type == SEDAM_DANA && key[i].time_spent < SEDAM_DANA) || key[i].key_type == NEOGRANICEN)
        {
            free(key);
            return 1;
        }
    ...
    if (changeKey)
        key[i].id = 'o'; //ako je aktivni kljuc potrosen, stavlja mu se marker "o"-old
    //SMINKANJE
    ...
    {
        char *cp;
        if (!strcmp(tempkey, cp = decrypt(key[i].encrypted_key)) && key[i].game == igra)
            found = 1;
        free(cp);
    }
    free(tempkey);
...
char *decrypt(char *cryptedKey)
{
    ...
    for (int i = 0; i < strlen(temp); i++)
        temp[i] -= POMJERAJ; //testno blokiranje
    return temp;
}
...
void prikaziKljuceve(int x, int y)
{
    ...
    for (int i = 0; i < brojKljuceva; i++)
    {
        if (key[i].game == 1 && key[i].id == 'a' && key[i].time_spent < key[i].key_type)
            keyIndex[0] = i;
        else if (key[i].game == 2 && key[i].id == 'a' && key[i].time_spent < key[i].key_type)
            keyIndex[1] = i;
        else if (key[i].game == 3 && key[i].id == 'a' && key[i].time_spent < key[i].key_type)
            keyIndex[2] = i;
        else if (key[i].game == 4 && key[i].id == 'a')
            keyIndex[3] = i;
    }
    changeTextColor(YELLOW);
    gotoxy(x + 28, 0);
    printf("LISTA AKTIVNIH KLJUCEVA");
    ...
    for (int i = 0; i < 4; i++)
    {
        if (keyIndex[i] != -1)
        {
            ...
            {
                gotoxy(x + 41, y + 8 + i * 4);
                if (tempTS < 0.05 * key[keyIndex[i]].key_type)
                    changeTextColor(RED);
...
void printTableKey(int x, int y)
{
    ...
    if ((fp = fopen("table.txt", "r")) != NULL)
    {
        ...
        printf("Greska kod otvaranja tabele!");
    gotoxy(x + 5, y + 2);
    printf("IGRA");
    gotoxy(x + 22, y + 2);
    printf("KLJUC");
    gotoxy(x + 38, y + 2);
    printf("PREOSTALO VRIJEME");
    gotoxy(x + 60, y + 2);
    printf("TIP KLJUCA");
    gotoxy(x + 2, y + 6);
    printf("Pogadjanje");
...
void otkaziKljuc(int igra)
{
    ...
    if (found)
    {
        ...
        printf("Kljuc za %d. igru je uspjesno otkazan!\n", igra);
        changeTextColor(DEFAULT);
    }
    else
        printf("Nema aktivnog kljuca za %d. igru. Otkazivanje neuspjesno!\n", igra);
    writeKeys(key, &brojKljuceva);
    Sleep(2000);
    system("cls");
}
...
int unosOtkazivanje(int (*pf)(char *, int *))
{
    ...
    do
    {
        if (pf == kontrolaUnosaOtk1)
            printf("Da li zelite da otkazete kljuc? (1-DA, 0-NE) : ");
        else if (pf == kontrolaUnosaOtk2)
            printf("Unesite broj igre ciji kljuc zelite da otkazete:\nPogadjanje - 1 \nKviz       - 2 \nLoto       - 3\nVjesala    - 4\nIzlaz      - 0 \nUnos: ");
