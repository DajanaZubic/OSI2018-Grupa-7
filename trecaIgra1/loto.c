...
int normal3(int *nizBrojeva)
{
    ...
    for(int i=0; i<20; i++)
    {
        ...
        if(i==0 || (nizIzvucenih[i-1]/10)==0) gotoxy(x+=2, 2);
        else gotoxy(x+=3, 2);
    ...
    for(int i=0; i<pogodjeni; i++) suma+=(i+1)*10;
...
void unos3(int *nizBrojeva)
{
    for(int i=0; i<7; i++)
    {
        ...
        system("cls");
        printf("Odabrali ste: ");
        for(int j=0; nizBrojeva[j] && j<7; j++) printf("%d ", nizBrojeva[j]);
    }
}
...
int controlled3(int *nizBrojeva, int brojPogodjenih)
{
    ...
    if((nizPogInd=(int*)calloc(brojPogodjenih, sizeof(int)))==NULL) {printf("Bad alloc!\n"); return 0;};
    if((nizPlaceInd=(int*)calloc(brojPogodjenih, sizeof(int)))==NULL) {printf("Bad alloc!\n"); return 0;};
    ...
    for(int i=0; i<brojPogodjenih; i++) suma+=(i+1)*10;
...
int playLoto(int izgubljeniBodoviIns, int *ukupniIzgubljeniBodovi, int *ukupniDobijeniBodovi)
{
    ...
    unos3(nizBrojeva);
    if(izgubljeniBodoviIns==INT_MAX) dobijeniBodovi=normal3(nizBrojeva);
    else{
